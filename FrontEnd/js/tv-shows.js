(function () {
    ...
    if (grid) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column: 1 / -1;">
          <h4>Unable to load TV shows</h4>
          <p>${message || 'Please check your connection and try again.'}</p>
          <button class="btn btn-primary" type="button" onclick="window.CineJunction.tvShowsPage?.retry?.()">Retry</button>
        </div>
      `;
    }
  }
  ...
  async function fetchShows(reset = true) {
    ...
    try {
      ...
      console.log('[TV Shows] Fetching...', params);
      const response = await window.CineJunction.movieService.getMovies(params);
      console.log('[TV Shows] API Success. Total elements:', response.totalElements);
      ...
      if (state.items.length === 0) {
        showEmpty('No TV shows match your current filters.');
      } else {
  ...
  async function performSearch(query) {
    ...
    try {
      console.log('[TV Shows] Searching for:', query);
      const response = await window.CineJunction.movieService.searchMovies(query, state.currentPage, 12);
      console.log('[TV Shows] Search API Success. Results:', response.totalElements);
      ...
      if (state.items.length === 0) {
        showEmpty('No TV shows match your search.');
      } else {
        renderMovies(state.items);
        renderPagination(state.currentPage, state.totalPages, state.totalElements);
      }
      ...
      console.error('[TV Shows] Search API Error:', error);
      state.error = error;
      if (error.status === 401) {
        const isInPages = window.location.pathname.indexOf('/pages/') !== -1;
        window.location.href = (isInPages ? 'login.html' : 'pages/login.html');
  ...
  async function initPage() {
    console.log('[TV Shows] Loading...');
    bindEvents();
    await fetchShows();
    console.log('[TV Shows] Completed');
  }

  window.CineJunction.tvShowsPage = {
    init: initPage,
    retry: () => fetchShows()
  };
