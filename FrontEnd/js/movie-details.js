(function () {
  ...
  function renderHeroStats(movie) {
    const stats = document.querySelectorAll('.hero-stat strong');
    if (stats.length >= 4) {
      stats[0].textContent = movie.averageRating ? movie.averageRating.toFixed(1) : 'N/A';
      stats[1].textContent = 'N/A';
      stats[2].textContent = movie.popularity ? Math.round(Number(movie.popularity)) + '%' : 'N/A';
      stats[3].textContent = 'N/A';
    }
  }
  ...
  function renderRatings(movie) {
    ...
    const ratingsGrid = document.querySelector('.ratings-grid');
    if (ratingsGrid) {
      const avgRating = movie.averageRating ? movie.averageRating.toFixed(1) : 'N/A';
      const voteCount = movie.voteCount || 'N/A';
      const popularity = movie.popularity ? Math.round(Number(movie.popularity)) + '%' : 'N/A';
      ...
      ratingsGrid.innerHTML = `
        <article class="ratings-card">
          <div class="rating-overview">
            ...
            <div class="score-pill">${voteCount !== 'N/A' ? voteCount + ' votes' : ''}</div>
          </div>
          <ul class="score-list" aria-label="External ratings">
            <li><span>IMDb</span><strong>${avgRating}</strong></li>
            <li><span>Vote Count</span><strong>${voteCount}</strong></li>
            <li><span>Popularity</span><strong>${popularity}</strong></li>
            <li><span>Audience</span><strong>N/A</strong></li>
          </ul>
        </article>
        <article class="ratings-card">
          <h3 class="movie-title">Rating Details</h3>
          <ul class="rating-chart" aria-label="Score distribution">
            <li><span>5★</span><div class="bar"><i style="width: 0%"></i></div><strong>0%</strong></li>
            <li><span>4★</span><div class="bar"><i style="width: 0%"></i></div><strong>0%</strong></li>
            <li><span>3★</span><div class="bar"><i style="width: 0%"></i></div><strong>0%</strong></li>
            <li><span>2★</span><div class="bar"><i style="width: 0%"></i></div><strong>0%</strong></li>
            <li><span>1★</span><div class="bar"><i style="width: 0%"></i></div><strong>0%</strong></li>
          </ul>
        </article>
      `;
    }
