(function () {
    ...
    if (grid) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column: 1 / -1;">
          <h4>Unable to load trending titles</h4>
          <p>${message || 'Please check your connection and try again.'}</p>
          <button class="btn btn-primary" type="button" onclick="window.CineJunction.trendingPage?.retry?.()">Retry</button>
        </div>
      `;
    }
  }
  ...
  function showEmpty(message) {
    ...
    if (grid) {
      grid.innerHTML = `
        <div class="empty-state" style="grid-column: 1 / -1;">
          <h4>No trending titles found</h4>
  ...
  async function fetchTrending(reset = true) {
    ...
    try {
      ...
      console.log('[Trending] Fetching...', params);
      const response = await window.CineJunction.movieService.getMovies(params);
      console.log('[Trending] API Success. Total elements:', response.totalElements);
      ...
      if (state.items.length === 0) {
        showEmpty('No trending titles match your current filters.');
      } else {
      ...
      if (error.status === 401) {
        ...
        showEmpty('No trending titles found.');
      } else {
  ...
  async function performSearch(query) {
    ...
    try {
      console.log('[Trending] Searching for:', query);
      const response = await window.CineJunction.movieService.searchMovies(query, state.currentPage, 12);
      console.log('[Trending] Search API Success. Results:', response.totalElements);
      ...
      if (state.items.length === 0) {
        showEmpty('No trending titles match your search.');
      } else {
        renderTrending(state.items);
        renderPagination(state.currentPage, state.totalPages, state.totalElements);
      }
  ...
  async function initPage() {
    ...
    bindEvents();
    await fetchTrending();
    console.log('[Trending] Completed');
  }

  window.CineJunction.trendingPage = {
    init: initPage,
    retry: () => fetchTrending()
  };
